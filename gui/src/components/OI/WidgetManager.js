...
class WidgetManager extends React.Component {
        ...
        ? action[key].name === "toggleActivate"
          ? actionButtons.push(
              <abbr className={this.checkedList[e.name] ? "deactivateWidget" : "activateWidget"} title={this.checkedList[e.name] ? "Deactivate" : "Activate"} key={index}>
                <Switch id={e.name} onChange={() => this.buttonAction(action[key], e)} checked={this.state.checked[e.name]} onColor='#86d3ff' onHandleColor='#2693e6' handleDiameter={10} uncheckedIcon={false} checkedIcon={false} boxShadow='0px 1px 5px rgba(0, 0, 0, 0.6)' activeBoxShadow='0px 0px 1px 10px rgba(0, 0, 0, 0.2)' height={20} width={33} className='react-switch' />
              </abbr>
            )
          : actionButtons.push(
              <abbr title={action[key].name} key={index}>
                <Button className=' btn manage-btn k-grid-edit-command' variant={variant} onClick={() => this.buttonAction(action[key], e)} style={buttonStyles}>
  ...
  buttonAction(action, item) {
    if (action.name !== undefined) {
      if (action.name === "toggleActivate" && item.isdeleted == "0") this.widgetOperation(item, "Delete");
      else if (action.name === "toggleActivate" && item.isdeleted == "1") this.widgetOperation(item, "Activate");
      else this.widgetOperation(item, action.name);
    }
  }
  ...
  render() {
    return (
      <div className='widget-manager'>
        ...
        <WidgetModal osjsCore={this.core} modalType={this.state.modalType} show={this.state.showModal} onHide={() => this.setState({ showModal: false })} content={this.state.modalContent} handleChange={(e) => this.handleChange(e)} notification={this.notif} refreshGrid={this.refresh} />
      </div>
    );
  }
}
