...
export default class WidgetGridNew extends React.Component {
  constructor(props) {
    ...
    this.columnConfig = configuration
      ? configuration.column
        ...
        : []
      : [];
    this.rightClickPopup = configuration
      ? configuration.rightClickPopup
        ? configuration.rightClickPopup
        : false
      : false;
  ...
  saveAsExcel = () => {
    let filterData;
    filterData =
      Object.keys(this.state.displayedData.data).length > 0
        ? this.state.displayedData.data
        : this.allData;
  ...
  parseData = () => {
    ...
    for (const config of this.columnConfig) {
      if (config["dataType"] == "date" || config["type"] == "date") {
        fieldDataTypeMap.set(
          config["field"],
          config["dataType"] || config["type"]
        );
      }
    }
  ...
  drillDownClick = (evt) => {
    ...
    let drillDownTarget =
      this.state.props.configuration["oxzion-meta"]["drillDown"]["target"];
    let drillDownField =
    ...
    if (drillDownTarget == "file") {
      ...
      WidgetDrillDownHelper.drillDownClicked(
        WidgetDrillDownHelper.findWidgetElement(
          evt.nativeEvent ? evt.nativeEvent.target : evt.target
        ),
        evt.dataItem
      );
  ...
  hasBackButton() {
    if (this.props.canvasElement && this.props.canvasElement.parentElement) {
      let backbutton =
        this.props.canvasElement.parentElement.getElementsByClassName(
          "oxzion-widget-roll-up-button"
        );
      if (backbutton.length > 0) return true;
      else return false;
    } else {
      return false;
    }
  ...
  cellRender(tdElement, cellProps, thiz) {
    ...
    if (cellProps.rowType === "groupFooter") {
      let element = null;
      if (
        thiz.props.configuration["groupable"] &&
        thiz.props.configuration["groupable"] != false &&
        thiz.props.configuration["groupable"]["aggregate"]
      ) {
  ...
  handleOnSelect = (e) => {
    ...
    switch (e.item.text) {
      case "View":
        this.launchApplication(this.dataItem,
          this.state.props.configuration["oxzion-meta"]["drillDown"][
            "nextWidgetId"
          ],
          "file_uuid"
  ...
  deleteRecord = async () => {
    ...
    // const fileResponse = this.restClient.request('v1', "/app/" + app.appId + "/file/" + file_uuid + "/data")
    //get file version before deleting it
    const { status, message } = await this.restClient.request(
      "v1",
      `app/${app.appId}/file/crud/${file_uuid}`, {}, "delete"
    );
    const isSuccess = status == "success";
    this.props.notif?.current?.notify(isSuccess ? "Success" : "Error",!isSuccess ? message : "Deleted Successfully", isSuccess ? "success" : "danger");
    if(isSuccess){
  ...
  render() {
        ...
        if (config["footerAggregate"]) {
          ...
          columns.push(
            <Column
              ...
              {...config}
              className={config["className"] ? config["className"] : null}
              cell={(props) => thiz.jsonToText(props, config)}
            />
          );
        } else {
          if (config["type"] == null) {
            ...
            columns.push(
              <Column
                ...
                {...config}
                className={config["className"] ? config["className"] : null}
                cell={(props) => thiz.myCustomDateCell(props, config)}
              />
            );
          } else {
            columns.push(
    ...
    let gridTag = (
      <Grid
        ...
        rowRender={this.rightClickPopup ? this.rowRender : this.rowRender}
