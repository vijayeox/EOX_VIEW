...
export default class DocumentSignerComponent extends Base {
    ...
    bindHandlers() {
      ...
      <div class="singleFile row" ` +
            ...
            url +
            `">
        <span class="fileName col-md-9">` +
            icon +
            "&nbsp;&nbsp;" +
            name +
            `</span> <span class="col-md-3"><button` +
          ...
          <button class="btn btn-sm btn-info ` +
            component.key +
            `-signFile" id="sign_btn_` +
            this.id +
            `" style="margin-left:5px;" title="Click to sign the document">
            ...
            component.key +
            `-filePreviewModal" class="modal"> <div style="height:inherit;display: block;background-color: white;"><div id="` +
            component.key +
            `-closeFile" class="viewer-button viewer-close" style="z-index:111"></div><div id="` +
            component.key +
            `-filePreviewWindow" style="height:inherit"></div></div></div>`;
        that.documentsList +=
            `<div id="myModal_` + this.id + `" class="insuresign modal"></div>`;
        that.redraw();
    }
    ...
    attach(element) {
        ...
        if (document.getElementById("sign_btn_" + this.id)) {
            document
                .getElementById("sign_btn_" + this.id)
                .addEventListener("click", async() => {
                    ...
                    this.showInsureSignModal(
                        this.component.documentsList + this.component.index,
                        signingLink
                    );
                    this.pollForStatus();
                });
        ...
        if (element) {
            ...
            Array.from(elements).forEach(function(ele) {
                ele.addEventListener("click", function(event) {
                    ...
                    if (fileType == "png" || fileType == "jpeg" || fileType == "jpg") {
                        document.getElementById(that.key + "-filePreviewWindow").innerHTML =
            ...
            var downloadElements = element.getElementsByClassName(
                that.key + "-downloadFile"
            );
    ...
    showInsureSignModal(docid, signing_url) {
        let modalHTML = "";
        modalHTML += window.open(signing_url, "_blank");
    }
    ...
    pollForStatus() {
        ...
        setTimeout(() => {
            var docId =
                that.form._data[that.component.documentsList][that.component.index]
                .docId;
            let helper = that.component.core.make("oxzion/restClient");
            helper
                .request("v1", "/esign/status/" + docId, {}, "get")
                .then(function(response) {
                    if (
                        ...
                        document.getElementById("myModal_" + that.id).style.display =
                            "none";
                        that.documentsList =
                            `<div class="docList" style="margin:0;" key="` +
                            that.id +
