  ...
  upload(files) {
    ...
    if (this.component.storage && files && files.length) {
      ...
      Array.prototype.forEach.call(files, function (file) {
        var fileName = (0, _utils.uniqueName)(
          file.name,
          _this6.component.fileNameTemplate,
          _this6.evalContext()
        );
        var fileUpload = {
        ...
        if (
          _this6.component.filePattern &&
          !_this6.validatePattern(file, _this6.component.filePattern)
        ) {
          fileUpload.status = "error";
          fileUpload.message = _this6.t(
        ...
        if (
          _this6.component.fileMinSize &&
          !_this6.validateMinSize(file, _this6.component.fileMinSize)
        ) {
          fileUpload.status = "error";
          fileUpload.message = _this6.t(
        ...
        if (
          _this6.component.fileMaxSize &&
          !_this6.validateMaxSize(file, _this6.component.fileMaxSize)
        ) {
          fileUpload.status = "error";
          fileUpload.message = _this6.t(
        ...
        if (fileUpload.status !== "error") {
          ...
          var _this6$component = _this6.component,
            ...
            options =
              _this6$component$opti === void 0 ? {} : _this6$component$opti;
          ...
          if (_this6.component.storage == "url") {
            ...
            helper
              .request(
                "v1",
                _this6.component.properties["absoluteUrl"]
                  ? url
                  : "/app/" + _this6.component.appId + _this6.component.url,
                uploadFile,
                "fileupload"
              )
      ...
      .then((file) => {
        if (file) {
          if (["base64", "indexeddb"].includes(file.storage)) {
            ...
            if (
              fileInfo.url &&
              !fileInfo.url.includes(
                this.component.wrapperUrl + this.component.appId,
                0
              )
            ) {
              window.open(
                fileInfo.url.replace(
                  this.component.uiUrl,
                  this.component.wrapperUrl + this.component.appId
                ),
                "_blank"
              );
