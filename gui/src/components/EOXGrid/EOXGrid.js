...
export default class EOXGrid extends React.Component {
	constructor(props) {
		...
		this.noCreateAction = this.props.noCreateAction
			? this.props.noCreateAction
			: false;
	...
	async handleCreateSubmit(formData, createFlag) {
		if (formData) {
			...
			Requests.createFormPushData(
				this.core,
				this.createApi,
				this.props.getCustomPayload?.(formData) || formData,
				this.props.createCrudType
			).then((response) => {
	...
	render() {
		let gridTag = (
			<div id="eox-grid" style={{ position: "relative", marginTop: "-32px" }}>
				...
				<div
					style={{ float: "right" }}
					id="dash-manager-button"
					className="dash-manager-buttons"
				>
