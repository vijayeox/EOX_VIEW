...
class InsureLearn extends Component {
  constructor(props) {
    super(props);
  ...
  generateToken() {
    ...
    let url = 'app/' + this.state.appUuid + '/delegate/InsureLearnSso/';
    if (this.state.config.email) {
      email += '?emailForSSO=' + this.state.config.email;
    }
